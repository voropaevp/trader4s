...
object DbSession {
  private[db] def runMigration[F[_]: Sync](keyspace: String): F[CqlSession] = makeMigrationSession.use { session =>
    for {
      _ <- Sync[F].delay(
      ...
      migratedSession <- Sync[F].delay(CqlSession.builder.withKeyspace(keyspace).build)
  ...
  def makeSession[F[_]: Sync](keyspace: String): Resource[F, CqlSession] =
    Resource.make(runMigration(keyspace))(session => Sync[F].delay(session.close()))
